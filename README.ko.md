# PingStats

PingStats는 macOS 메뉴바에서 동작하는 네이티브 앱입니다. 등록한 호스트로 주기적으로 ping을 보내고, 각 호스트의 상태를 메뉴바의 세로 컬러 바로 표시합니다.

[English README](README.md)

## 주요 기능

- 설정 창에서 여러 대상의 이름과 주소 등록.
- 주소 동작 방식:
  - `IP` 형식은 일반 ping으로 측정.
  - `IP:port` 형식은 TCP 연결 시간으로 측정.
- 백그라운드 ping 검사: 기본 주기 5초, 타임아웃 3초.
- 팝오버가 열린 동안 포어그라운드 ping 검사: 기본 주기 1초, 타임아웃 1초.
- 호스트별 메뉴바 상태 색상:
  - 초록: 최근 10건 평균이 설정 가능한 초록 임계값(기본 60 ms) 미만. 설정 창에서 변경.
  - 파랑: 최근 10건 평균이 설정 가능한 파랑 임계값(기본 120 ms) 미만. 설정 창에서 변경.
  - 노랑: 최근 10건 평균이 파랑 임계값 이상.
  - 주황: 최근 10건 중 타임아웃 또는 네트워크 오류 1건 이상.
  - 빨강: 최근 5건 중 오류 4건 이상.
  - 회색: 워밍업 상태로 샘플 10건 미만이며 아직 오류 없음.
- 메뉴바 배치: 대상 9개까지는 1행, 10개부터는 1 px 간격의 2행 격자로 배치. 왼쪽에서 오른쪽으로, 윗줄부터 채움.
- 대상별 알림 레벨(설정 창에서 지정):
  - `Warning`은 노랑, `Error`는 주황과 빨강, `All`은 둘 다 활성화.
  - 체크를 해제한 레벨은 해당 대상에서 알리지 않음. 그 상태에서 정상으로 복구되는 알림도 함께 제외.
- 알림 조건(대상별 알림 레벨을 통과한 경우에 한함):
  - 상태 범주가 바뀔 때: 정상(초록/파랑)에서 경고/오류 상태로, 그리고 정상으로 복구될 때.
  - 노랑/주황/빨강 사이에서 단계가 변경될 때.
  - 워밍업이 끝나고 정상(회색에서 초록/파랑)으로 안정화되면 알림.
  - 초록과 파랑 사이의 정상 범주 전환, 다시 워밍업 회색 상태로 들어가는 전환, 시작 시점부터 다운인 대상(회색에서 곧바로 오류 상태로)은 알림 없음. 이후 복구될 때만 알림.
- 메뉴바 클릭 시 2열 차트 팝오버 표시.
- 팝오버에서 현재 시간(`HH:mm:ss`)과 최근 5분 응답 시간 추이를 표시.
- 팝오버 상단에 내 IP 표시: 활성 상태이며 loopback이 아닌 모든 인터페이스(Wi-Fi, 이더넷,
  VPN 터널 등)의 라우팅 가능한 주소를 네트워크 설정과 같은 이름으로 표시. IPv4 우선,
  link-local(`169.254.*`, `fe80::`) 주소는 제외. 주소를 클릭하면 복사.
- 설정 창의 `로그인 시 실행` 체크박스: `SMAppService` 기반이며 저장 버튼과 무관하게 즉시 적용.
  `.app` 번들이 필요하므로 `swift run` 실행 시에는 비활성. macOS가 승인을 요구하면 설정 행에서
  시스템 설정 > 일반 > 로그인 항목으로 바로 이동.
- 시스템 언어에 따라 한국어/영어 UI 제공. 번역은 `Resources/ko.lproj/Localizable.strings`에 있으며
  `scripts/build-app.sh`가 번들에 복사. 번들 없이 실행하면 영어로 표시.

## 빌드

이 프로젝트는 외부 의존성 없이 Swift Package Manager를 사용합니다.

```sh
swift build
swift run PingStats
```

메뉴바 `.app` 번들을 만들려면:

```sh
chmod +x scripts/build-app.sh
scripts/build-app.sh
open build/PingStats.app
```

## 셀프 서명 배포 패키지

로컬 셀프 서명 코드서명 인증서로 앱을 서명하고 zip 패키지를 만들려면:

```sh
scripts/sign-self-signed.sh
```

셀프 서명 인증서가 아직 코드서명용으로 신뢰되지 않았다면 명시적으로 로컬 신뢰 설정을 추가해야 합니다.

```sh
scripts/sign-self-signed.sh --trust-local
```

이 명령은 로그인 키체인의 trust 설정을 변경합니다. 스크립트는 `dist/PingStats.zip`과 `dist/PingStatsSelfSigned.cer`를 생성합니다.

다른 Mac에서는 셀프 서명 인증서가 자동으로 신뢰되지 않습니다. 최초 실행 시 Control-click > Open을 사용하거나, `dist/PingStatsSelfSigned.cer`를 키체인 접근 앱에 가져온 뒤 코드서명용으로 신뢰해야 합니다.

GitHub Actions workflow는 CI 릴리즈 패키지에 ad-hoc 서명을 사용합니다. macOS에서 셀프 서명 인증서 trust 변경은 비대화형 러너에서 승인할 수 없는 키체인 권한 작업이기 때문입니다.

## Xcode 툴체인 검증

`swift build`가 SDK/compiler mismatch 오류로 실패하면 Xcode 또는 Command Line Tools 설정을 확인합니다.

```sh
xcode-select --install
sudo xcode-select -s /Applications/Xcode.app/Contents/Developer
```

Xcode 설치 후 아래 스크립트로 툴체인 전환과 프로젝트 빌드를 검증할 수 있습니다.

```sh
scripts/verify-xcode-toolchain.sh
```

## GitHub Actions

`.github/workflows/desktop-macos-ci.yml`은 `v*` 태그 push 또는 수동 실행 시 동작합니다.

- self-hosted macOS ARM64 러너에서 빌드.
- SwiftPM manifest 검증.
- `.app` 번들 생성.
- ad-hoc 서명 및 zip 패키지 생성.
- artifact 업로드.
- 태그 빌드인 경우 GitHub Release에 산출물 첨부.

현재 구현은 메뉴바 상태 아이템에 AppKit을 사용하고, 팝오버와 설정 화면에는 SwiftUI를 사용합니다.
