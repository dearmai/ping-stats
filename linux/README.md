...
확장이 없어도 앱 목록에서 모니터 창을 열 수 있습니다. 아이콘 메뉴에서 모니터,
설정, 종료를 선택합니다. 창을 닫으면 검사는 백그라운드에서 계속됩니다.
완전한 종료는 창 또는 아이콘 메뉴의 **종료**를 사용합니다.
...
클릭 즉시 같은 모양의 팝오버가 열리는 방식은 아닙니다. **모니터** 메뉴로 별도 GTK 창을 엽니다.
...
주소 해석, loopback ICMP/TCP/HTTP 및 HTTP 제한 시간을 확인합니다.
`../shared/health-cases.json`은 기존 Swift 규칙에서 옮긴 공통 검증 데이터입니다.
Swift 테스트 실행과 자동 비교까지 연결된 상태는 아닙니다.
GUI 검증은 임시 설정과 loopback 대상으로 창·차트·설정·아이콘 생성·창 숨김/재표시를 확인합니다.
